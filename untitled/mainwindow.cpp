...
Alistador::Alistador(listaArticulos *_listaArt, int _id, QVector<QVector<QString>> matriz, QObject* parent)
            : QThread(parent), id(_id), listaArt(_listaArt), matrizAlistador(matriz)
        {
        ...
        QVector<QVector<QString>> Alistador::getMatrizAlistador(){
                return matrizAlistador;
        }
        ...
        void Alistador::agregarDato(int fila, int columna, const QString& dato){
                QMutexLocker locker (&mutex);

                if (fila >= 0 && fila<matrizAlistador.size() && columna >= 0 && columna < matrizAlistador[fila].size()){
                    matrizAlistador[fila][columna] = dato;
                } else{
                    qDebug() << "No se añadió a la matriz";
                }
        }
        ...
        void Alistador::moverAlistador(ArticuloPedido* articulo)
        {
                ...
                agregarDato(fila, columna, QString::fromStdString(articulo->codProd));
...
Bodega::Bodega(listaArticulos *_listaArt, Queue<Pedido*>& _colaAlisto, Queue<Pedido*>& _colaAlistados, Queue<Alistador *>& _colaAlistadores, QObject* parent)
            : listaArt(_listaArt),colaAlisto(_colaAlisto), colaAlistados(_colaAlistados), colaAlistadores(_colaAlistadores), QThread(parent)
        {
            ...
            QVector<QVector<QString>> matriz(10, QVector<QString>(27));
            ...
            //------------
            alistador1 = new Alistador(listaArt, 1, matriz);
            ...
            //------------
            alistador2 = new Alistador(listaArt, 2, matriz);
            ...
            //------------
            alistador3 = new Alistador(listaArt, 3, matriz);
            ...
            //------------
            alistador4 = new Alistador(listaArt, 4, matriz);
            ...
            //------------
            alistador5 = new Alistador(listaArt, 5, matriz);
            ...
            //------------
            alistador6 = new Alistador(listaArt, 6, matriz);
    ...
    /*void Bodega::agregarPedidoAlistado(Pedido* pedido)
    {
        ...
        QVector<QVector<QString>> Bodega::sumarMatrices(const QVector<QVector<QString>>& matriz1, const QVector<QVector<QString>>& matriz2)
        {
            QVector<QVector<QString>> matrizSuma(10, QVector<QString>(27));

            // Verificar que las dimensiones de las matrices sean iguales
            if (matriz1.size() != matriz2.size() || matriz1[0].size() != matriz2[0].size()) {
                    qDebug() << "Las dimensiones de las matrices no son compatibles";
                    return matrizSuma;
            }

            // Realizar la suma elemento por elemento
            for (int fila = 0; fila < matriz1.size(); fila++) {
                    for (int columna = 0; columna < matriz1[fila].size(); columna++) {
                        if (matriz1[fila][columna].isEmpty() && matriz2[fila][columna].isEmpty()) {
                            matrizSuma[fila][columna] = "";
                            //qDebug()<< "entra al primer if de sumarMatrices";
                        } else if (matriz1[fila][columna] != "" && matriz2[fila][columna] == "") {
                            matrizSuma[fila][columna] = matriz1[fila][columna];
                            //qDebug()<< "entra al segundo if de sumarMatrices";
                        } else if (matriz1[fila][columna] == "" && matriz2[fila][columna] != "") {
                            matrizSuma[fila][columna] = matriz2[fila][columna];
                            //qDebug()<< "entra al tercer if de sumarMatrices";
                        }
                    }
            }
    ...
    QVector<QVector<QString>> Bodega::getBodegaMatriz()
    {
            matriz = sumarMatrices(matriz, alistador1->getMatrizAlistador());
        return matriz;
    }
...
MainWindow::MainWindow(PriorityQueue* _colaPedidos, Queue<Pedido *> & _colaAlisto, Queue<Pedido *> & _colaAlistados,  Queue<Pedido *> & _colaA,  Queue<Pedido *> & _colaB,  Queue<Pedido *> & _colaC,  Queue<Pedido *> & _colaComodin, Queue<Alistador *> & _colaAlistadores,
        ...
        listaArt(la),lista(lc),alistad(a),revisor(r),balanceador(bl),A(f1),B(f2),C(f3),Comodin(f4),facturadora(fc),bodega(b),colaFacturacion(_colaFacturacion),
...
{
     QVector<QVector<QString>> matriz = bodega->getBodegaMatriz();
     int rowCount = matriz.size();
     int colCount = (rowCount > 0) ? matriz[0].size() : 0;

     // Actualizar la tabla en MainWindow con los datos de la matriz
     ui->tblBodega->setRowCount(rowCount);
     ui->tblBodega->setColumnCount(colCount);

     // Copiar los datos de la matriz a la tabla en MainWindow
     for (int row = 0; row < rowCount; row++)
     {
            for (int col = 0; col < colCount; col++)
            {
                QTableWidgetItem* newItem = new QTableWidgetItem(matriz[row][col]);
                ui->tblBodega->setItem(row, col, newItem);
                //qDebug() << "posicion en tabla actualizada";
            }
     }
